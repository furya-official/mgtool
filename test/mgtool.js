...
const setup = async (mageEndpoint, binanceEndpoint, mageMnemonic, binanceMnemonic) => {
    // Start new Mage client
    mageClient = new Mage.MageClient(mageEndpoint);
...
const loadMageDeputies = async (mageClient, assets, amount) => {
    var counter = 0;
    for (var denom in assets) {
        const assetInfo = assets[denom];
        const coins = Mage.utils.formatCoins(amount * assetInfo.conversionFactor, assetInfo.mageDenom);
