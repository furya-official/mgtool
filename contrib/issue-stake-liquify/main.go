...
import (
	...
	"github.com/furya-official/mgtool/contrib/issue-stake-liquify/config"
...
// - fund each delegator account with the required amount of Mage (via dev-wallet issuing)
...
func ProcessDelegationAllocations(cfg config.Config, allocations config.Allocations) error {
	...
	makeSigner := SignerFactory(cfg.ChainID, cfg.MageGrpcEndpoint)
