...
- a mgd node, bnbchain node (with rest servers), and the deputy for integration testing the web app
- a bare mgd node built from unmerged branches for helping with PR reviews
- a local mgd node with genesis that closely mirrors mainnet to test out upcoming gov proposals
- a mgd node and oracle to test out CDP top up bot
- etc
...
- services need to be packaged into docker containers (although for the web app, it could run outside docker while interacting with mage/bnb rest servers running in docker)
...
Currently there are a load of manually created files in `config_templates` which are copied and lightly edited when `mgtool testnet` runs. This works for now but in the future moving all of the data into go packages will be more scalable.
